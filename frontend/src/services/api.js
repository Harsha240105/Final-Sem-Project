...
export async function getCommunityLeaderboard(communityId, token) {
  return request(apiClient.get(`/communities/${communityId}/leaderboard`, withAuth(token)));
}
...
export async function getLeaderboard(token, page = 1, limit = 20, role = "") {
  const query = `?page=${page}&limit=${limit}${role ? `&role=${encodeURIComponent(role)}` : ""}`;
  return request(apiClient.get(`/social/leaderboard${query}`, withAuth(token)));
}
