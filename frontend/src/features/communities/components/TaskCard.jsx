...
function TaskCard({ task, isAdmin, isArchived, onMarkComplete, onUploadFile, submitting, onClick }) {
  ...
  const isStudentCompleted = (task.completedBy || []).some(
    (entry) => (entry.userId?._id || entry.userId) === currentUserId
  );
  ...
  return (
    ...
    >
      {/* Left color accent */}
      <div className={`h-full w-1 shrink-0 ${task.completed_status || isStudentCompleted ? "bg-emerald-500" : "bg-purple-500/60"}`} />

      {/* Icon */}
      <div className={`shrink-0 flex h-8 w-8 items-center justify-center rounded-lg ${
        task.completed_status || isStudentCompleted ? "bg-emerald-500/15 text-emerald-400" : "bg-purple-500/15 text-purple-400"
      }`}>
        {task.completed_status || isStudentCompleted ? (
      ...
      <div className="flex-1 min-w-0 py-3">
        <div className="flex items-center gap-2">
          <p className={`text-sm font-semibold truncate transition ${task.completed_status || isStudentCompleted ? "text-emerald-300/70 line-through" : "text-white group-hover:text-gray-100"}`}>
            {task.title}
          </p>
          {!task.completed_status && !isStudentCompleted && <span className="shrink-0 rounded-full px-2 py-0.5 text-[8px] font-bold bg-amber-500/10 border border-amber-500/20 text-amber-300">Pending</span>}
        </div>
