...
import { connectLiveEvents } from "../utils/liveEvents";
...
export default function Discover() {
  ...
  useEffect(() => {
    fetchUsers();
    const token = localStorage.getItem("token");
    if (token) connectLiveEvents(token);
    const handler = () => fetchUsers();
    window.addEventListener("live:followCountUpdate", handler);
    return () => window.removeEventListener("live:followCountUpdate", handler);
  }, [fetchUsers]);
